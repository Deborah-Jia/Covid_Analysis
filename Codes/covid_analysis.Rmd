...
* Substantive reasoning:
  * Log transfomation of cases may remain valid in spite of pandemic waves and other seasonal fluctuations.
...
* The final model is ln(death)~ln(confirmed), which reveals death cases is 1.03 percent higher, on average, for observations having 1 percent higher confirmed cases.
...
![model comparision](/Users/wodediannao/Desktop/ECBS-5208-Coding-1-Business-Analytics/assignment_covid/Out/model_comparison.png)

## Model Summaries

With model comparision table and plot visualization, we can conclude:
...
* For quadratic regressioin:
  * 0.02 is positive; so the relationship is convex
  * r-square = 0.89 means 89% of the variation in ln(death cases) is captured by the regression, and 11% is left for residual variation.
* For Piecewise linear spline regression:
  * Among obervations with ln(confirmed_case) values less than 8, ln(death_cases) is 0.92 units higher, on average, for observations with one unit higher ln(confirmed_case) value.
  * Among obervations with ln(confirmed_case) values between 8 and 12, ln(death_cases) is 0.96 units higher, on average, for observations with one unit higher ln(confirmed_case) value.
  * Among obervations with ln(confirmed_case) values between 12 and 14, ln(death_cases) is 1.54 units higher, on average, for observations with one unit higher ln(confirmed_case) value.
  * Among obervations with ln(confirmed_case) values above 14, ln(death_cases) is 0.62 units higher, on average, for observations with one unit higher ln(confirmed_case) value.
  * For Weighted linear regression: USA, Brazil and India have most servere covid pandemic situation.
...
* Statistical: 
  * Conpared with other three, simple regression(log-log) model is easy to interpret.
  * Log-log model has comparatively high R-square and captures variation well.
